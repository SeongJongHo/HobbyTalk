...
public class OpenChatServiceTest {
    @Mock
    private IOpenChatRepository IOpenChatRepository;
    @InjectMocks
    ...
    class Describe_getLastOpenChatByOpenChatRoomId{
        ...
        void openChatRepository_selectLastOpenChatByChatRoomId_메서드를_호출하고_반환받은_값을_반환한다(){
            ...
            when(IOpenChatRepository.selectLastOpenChatByChatRoomId(chatRoomId)).thenReturn(openChat);

            // when
            Optional<OpenChat> result = openChatService.getLastOpenChatByOpenChatRoomId(chatRoomId);

            // then
            verify(IOpenChatRepository).selectLastOpenChatByChatRoomId(chatRoomId);
    ...
    class Describe_getUnReadOpenChatCountByOpenChatRoomIdAndLastExitTime{
        ...
        void openChatRepository_selectUnReadOpenChatCountByChatRoomIdAndLastExitTime_메서드를_호출하고_반환받은_값을_반환한다(){
            ...
            int limit = 150;
            when(IOpenChatRepository.selectUnReadOpenChatCountByChatRoomIdAndLastExitTime(chatRoomId, lastExitTime, limit)).thenReturn(unReadOpenChatCount);

            // when
            int result = openChatService.getUnReadOpenChatCountByOpenChatRoomIdAndLastExitTime(chatRoomId, lastExitTime, limit);

            // then
            verify(IOpenChatRepository).selectUnReadOpenChatCountByChatRoomIdAndLastExitTime(chatRoomId, lastExitTime, limit);
